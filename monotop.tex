...
\begin{align}
    ...
    & \phantom{\Big[} \times \prod_{X=\mu,e} \pois\left(\left.d^{X}_i~\Big|~ \frac{\muwi}{\Ti{X}{W}(\bm\theta)} + \frac{\muti}{\Ti{X}{\ttbar}(\bm\theta)} + B^{X}_i(\bm\theta) \right) \right]  \times  \prod_{j=0}^{n_\theta} p(\theta_j)
\end{align}
...
\end{equation}
We include another$k$-factor, $k_\mathrm{EWK}$, to cover NLO EWK effects.
