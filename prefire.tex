...
When an L1 seed is triggered to accept an event (\emph{L1A}), the following two bunch crossings are blocked from firing L1As. 
...
In what follows, we will refer to the bunch crossing with an interesting collision, the one we would like the trigger to select, as \bx{0}.
...
This ECAL L1 seed in \bx{-1}~will set to zero the corresponding ECAL clusters in \bx{0} (\emph{zero suppression}), further biasing the event description.
Thus, we would have an L1A for an arbitrary event (\bx{-1}), and the interesting event (\bx{0}) would be blocked from passing the L1 altogether. 
...
To measure how often an ECAL energy deposit causes an event to be lost by pre-firing, we need to compute the following probability:
\begin{equation}
...
Even if there is a mis-timed ECAL seed in \bx{0}, it will be blocked from firing an L1A in \bx{-1}, and therefore \bx{0}~is protected.
If some other object in \bx{0}~ passes L1 and HLT decisions, then \bx{0}~ will be recorded and can be studied.
...
Using the bunch crossing numbering in Figure~\ref{fig:vbf:pre3}, we re-define the probability:
\begin{equation}
...
As a simple correction, events with a jet in this crystal are rejected.
