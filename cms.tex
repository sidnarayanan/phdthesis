...
\begin{itemize}
    ...
    \item[$\eta = $]$-\log\nicefrac{\theta}{2}$ (pseudorapidity), where $\theta$ is the polar angle
\end{itemize}
...
Due to the bending of an electron's trajectory in the solenoidal $B$-field, bremsstrahlung photons will be emitted at similar values of $\eta$, but spread along $\phi$.
A ``supercluster'' is defined by clustering nearby ECAL energy depositions, allowing for a wider spread in $\phi$ than in $\eta$ (Figure~\ref{fig:cms:sc}).
The particle's EM energy is defined to be the weighted sum of the energies of all crystals in the supercluster, where the coefficients account for crystal-specific calibration effects.
